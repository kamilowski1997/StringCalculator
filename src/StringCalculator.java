...
public class StringCalculator {
	...
	int Add(String numbers) {
		...
		ArrayList<String> defaultDelimeters = new ArrayList<String>();
		if(numbers.startsWith("//")) {
			for(int i =2;i<numbers.indexOf("\n");i++) {
				if(numbers.charAt(i) ==']') {
					defaultDelimeters.add(numbers.substring(3,4));
					numbers = numbers.substring(0,2)+numbers.substring(5);
					i=2;
				}
			}
		...
		for(String i:defaultDelimeters) {
			numbers2 = Split(numbers2, i);
		}
